...
def sample_features():
    ...
    return pd.DataFrame({
        ...
        "points_scored_avg_3": [np.nan, np.nan] + list(np.random.uniform(20, 30, n_games - 2)),
        "points_scored_avg_5": [np.nan] * 4 + list(np.random.uniform(20, 30, n_games - 4)),
        "points_scored_avg_10": [np.nan] * 9 + list(np.random.uniform(20, 30, n_games - 9)),
...
def test_validate_rolling_window_integrity_fail():
    ...
    df = pd.DataFrame({
        ...
        "points_scored_avg_10": [np.nan, np.nan, 25.0, 26.0],  # Has values too early
    })
...
def test_validate_no_nulls_after_warmup_pass(sample_features):
    ...
    validate_no_nulls_after_warmup(sample_features, max_window=10, allowed_null_rate=0.3)
...
def test_look_ahead_bias_detection():
    ...
    df = pd.DataFrame({
        ...
        "points_scored_avg_5": list(np.random.uniform(20, 30, 10)),  # No NaN period!
    })
